...
private:
    ...
    void add_header(Buffer& buffer) const;
    void add_content(Buffer& buffer);

    // 发生错误时，将请求路径重定向到对应的错误 HTML 页面
    void error_html();
    // 当连错误页面文件都找不到时，回退生成一段纯文本的 HTML 错误提示
    void error_content(Buffer& buffer, const std::string& message) const;

    // 根据文件后缀获取对应的 MIME Type
    std::string get_file_type() const;
public:
    HttpResponse() : code(-1), is_keep_alive(false), path(""), root_dir(""), mm_file(nullptr), mm_file_stat({}) {}
    ~HttpResponse() { unmap_file(); }

    // 初始化响应对象状态
    void init(int _code, bool _is_keep_alive, const std::string& _path, const std::string& _root_dir);
    ...
    size_t get_file_len() const noexcept { return static_cast<size_t>(mm_file_stat.st_size); }
