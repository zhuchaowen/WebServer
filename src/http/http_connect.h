...
public:
    ...
    HttpConnect() : fd(-1), addr({}), is_close(true), iov_count(0), iov() {}
    ...
    const char* get_ip() const noexcept { return client_ip.c_str(); }

    // 判断当前连接是否已经被主动关闭
    bool is_closed() const noexcept { return is_close; }
    // 判断当前 HTTP 连接是否要求 Keep-Alive
    bool is_keep_alive() const noexcept { return request.is_keep_alive(); }
    // 判断是否已经将准备好的响应头和文件全部发送完毕
    bool is_write_complete() const noexcept { return iov[0].iov_len + iov[1].iov_len == 0; }
    // 解析是否彻底完成
    bool is_parse_finish() const noexcept { return request.is_finish(); }
};
